...
def calculate_stress(consumer, long_avg, short_avg, time_index=0):
    """
    Reads the latest message from Kafka, updates the running averages, and stores data for visualization.
    """
    ...
    if msg is None or msg.value() is None:
        return short_avg, long_avg, 0  # No update if no message

    if msg.error():
        return short_avg, long_avg, 0  # Ignore errors
    ...
    try:
        ...
        execution_duration = float(message_dict.get("execution_duration_ms", 0))
        mb_spilled = float(message_dict.get("mbytes_spilled", 0))
